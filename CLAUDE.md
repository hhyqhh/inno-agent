...
Tests run with `npm test` (`vitest run`, root script) and also execute in the release CI. The suite is 91 files (707 tests), including backend chat-stream/trace persistence coverage and web chat trace/timeline coverage, while remaining skewed toward `memory/l2`; coverage for channels/scheduler/terminal/L1/L3 is tracked in `docs/quality-remediation-plan.md`. The TypeScript build (`npm run build`) remains the primary sanity check. No ESLint or Prettier configuration exists.
...
- `permission-system-config.ts` — managed default policy for `@gotgenes/pi-permission-system` written to `<configDir>/extensions/pi-permission-system/config.json` on first run (`"*": allow` fallback so inno's own tools stay silent, bash asks with a read-only allowlist and destructive denylist, sensitive-path denies, computer-use side-effect tools (`act_ui`/`launch_browser`/`navigate_browser`/`evaluate_browser`) ask while passive observation stays allowed, `authorizerChain: ["inno-web"]`); also `resolvePluginFile` for jiti-loading packages whose exports map blocks the extension entry subpath. Gated by `plugins.permissionSystem.enabled` (default on). Three policy modes (`plugins.permissionSystem.mode`: `default` / `auto` / `yolo`, default `default`) are built by `buildPermissionPolicy` and written on demand by `writePermissionPolicyConfig` from the settings endpoint — `auto` flips bash `"*"` to allow, `yolo` sets `yoloMode`, and every mode keeps the destructive-command and sensitive-path deny floor (yoloMode rewrites only `ask`, never `deny`).
- `computer-use` (bundled `@injaneity/pi-computer-use`, loaded in `inno-extension.ts` section 12) — desktop GUI control: `find_roots` / `observe_ui` / `search_ui` / `expand_ui` / `inspect_ui` / `read_text` / `wait_for` (passive observation) plus `act_ui` / `launch_browser` / `navigate_browser` / `evaluate_browser` (side effects, gated to `ask` by the permission policy). Accessibility-tree-first (semantic outlines, screenshots only as fallback). Gate: `isComputerUseEnabled` — `plugins.computerUse.enabled` overrides; unset means on only when `INNO_DESKTOP=1` (set by `electron/main.js`), so online deployments stay off. TS-only, jiti-loaded, no pi-ai alias needed. On macOS the signed helper installs to `~/Applications/pi-computer-use.app` on first tool use and needs Accessibility + Screen Recording grants; headless sessions cannot prompt, so a missing grant surfaces as a tool error with grant instructions. Plugin config: `<configDir>/extensions/pi-computer-use.json` or `<workspace>/.pi/computer-use.json` (browser_use / headless / cursor_overlay / managed_browser).
