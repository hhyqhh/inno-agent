...
- **Deployment defaults to loopback-only, not "no auth model" writ large.** The single-user threat model above stops at the process boundary — a LAN attacker isn't "another user" of the tool, so the server binds `127.0.0.1` by default (every unauthenticated `/api/*` route, including the terminal WebSocket, would otherwise be reachable by anyone who can reach the host). Set `server.host` + `server.token` in config.json if you specifically need remote access. See [issue #159](https://github.com/hhyqhh/inno-agent/issues/159).
