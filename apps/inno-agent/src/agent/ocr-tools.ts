...
function resolveLocalPath(filePath: string, workspaceDir: string): string | undefined {
...
export function createOcrTools(configHolder: ConfigHolder): ToolDefinition[] {
	const tool = defineTool({
		...
		parameters: Type.Object({
			filePath: Type.String({ description: "要识别的图片路径（工作区相对路径）或 http(s) URL" }),
		}),
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (!config) {
				return {
					content: [{ type: "text" as const, text: "尚未配置 OCR API token。请在设置面板的「OCR API」卡片填入 token 后重试。" }],
					details: { error: "ocr_not_configured" } as Record<string, unknown>,
				};
			}

			const input = isHttpUrl(filePath) ? filePath : resolveLocalPath(filePath, ctx.cwd);
			if (!input) {
				return {
