...
export function resolvePaddleOcrConfig(
	config: InnoConfig["ocrApi"] | undefined,
): PaddleOcrConfig | undefined {
...
/**
 * Resolve the OCR credentials used by non-agent flows such as Notebook/L2
 * archival. Settings writes are persisted before these flows run, so reading
 * the runtime config here keeps them aligned with the live agent config.
 */
...
async function submitJob(config: PaddleOcrConfig, input: string, signal: AbortSignal): Promise<string> {
	...
	if (!response.ok) {
		throw new PaddleOcrError(
			`OCR 任务提交失败：${response.status} ${text.slice(0, 500)}`,
			"SUBMIT_FAILED",
		);
	}
...
async function pollJob(config: PaddleOcrConfig, jobId: string, signal: AbortSignal): Promise<JobStatus> {
	...
	if (!response.ok) {
		throw new PaddleOcrError(
			`OCR 状态查询失败：${response.status} ${text.slice(0, 500)}`,
			"JOB_FAILED",
		);
	}
	const data = JSON.parse(text) as { data?: JobStatus };
	if (!data.data) {
		throw new PaddleOcrError(`OCR 状态响应缺少 data：${text.slice(0, 300)}`, "JOB_FAILED");
	}
	return data.data;
}
...
async function fetchResult(jsonUrl: string, signal: AbortSignal): Promise<string[]> {
	...
	if (!response.ok) {
		throw new PaddleOcrError(`OCR 结果下载失败：${response.status}`, "MISSING_RESULT");
	}
...
export async function recognizeWithPaddleOcr(
	...
	if (options.signal?.aborted) {
		throw new PaddleOcrError("OCR 任务已取消或超时", "ABORTED");
	}
	...
	try {
		...
		if (!jsonUrl) {
			throw new PaddleOcrError("OCR 任务完成但未返回结果 URL", "MISSING_RESULT");
		}
		const pages = await fetchResult(jsonUrl, controller.signal);
		if (pages.length === 0) {
			throw new PaddleOcrError("OCR 完成，但未识别到文字", "EMPTY_RESULT");
		}
		return {
			jobId,
			pages,
