...
const server = createServer(async (req, res) => {
	...
	try {
		...
		if (method === "PUT" && url === "/api/settings/memory") {
			const body = (await readBody(req)) as Record<string, unknown>;
			const keys = ["l1Enabled", "l2Enabled", "l3Enabled", "autoReviewEnabled"] as const;
			if (keys.every((k) => typeof body[k] !== "boolean")) {
				json(res, 400, { error: "Provide at least one of l1Enabled / l2Enabled / l3Enabled / autoReviewEnabled (boolean)" });
				return;
			}
			for (const k of keys) {
			...
			const current = config.memory ?? { l1Enabled: true, l2Enabled: true, l3Enabled: true, autoReviewEnabled: true };
			config.memory = {
				...
				autoReviewEnabled: typeof body.autoReviewEnabled === "boolean" ? body.autoReviewEnabled : current.autoReviewEnabled ?? true,
			};
			config = saveConfig(paths.configPath, config);
			syncConfig(config);
			json(res, 200, buildSafeSettings());
