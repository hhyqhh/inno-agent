...
import type { ChatNoteReference, QuestionnaireResult, StreamEventEnvelope, StreamSnapshot } from "../types/chat.js";
...
export async function postChat(prompt: string, sessionId?: string | null, images?: InlineImage[], noteReferences?: ChatNoteReference[]): Promise<string> {
	const res = await apiFetch<{ response: string }>("/api/chat", {
		method: "POST",
		body: JSON.stringify({ prompt, sessionId: sessionId ?? undefined, images: images?.length ? images : undefined, noteContext: noteReferences?.length ? { rawPaths: noteReferences.map((note) => note.rawPath) } : undefined }),
	});
	return res.response;
}

export function streamChat(prompt: string, sessionId: string, clientRequestId: string, signal?: AbortSignal, images?: InlineImage[], noteReferences?: ChatNoteReference[]): AsyncGenerator<StreamEventEnvelope> {
	return streamSSE<StreamEventEnvelope>("/api/chat/stream", { prompt, sessionId, clientRequestId, images: images?.length ? images : undefined, noteContext: noteReferences?.length ? { rawPaths: noteReferences.map((note) => note.rawPath) } : undefined }, signal);
}
