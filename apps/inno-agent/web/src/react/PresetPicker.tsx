...
/** Simple Mode preset grid with search and refresh. */
export function PresetPicker({
	...
	const groups = useMemo(
		() => groupByCategory(presets.filter((preset) => matchesQuery(preset, query, preset.category ? t(`categories.${preset.category}`, preset.category) : undefined)), uncategorizedLabel),
	...
	const refreshStatusLabel = refreshStatus === "success"
		? t("presets.refreshSucceeded")
		: t("presets.refreshFailed");
	...
	return (
		<div className="mt-5">
			...
			{showSearch ? (
				<div className="mb-2 flex items-center gap-2 rounded-md border border-[var(--inno-border)] bg-[var(--inno-surface)] px-2 py-1.5">
					...
					<input
						...
						className="min-w-0 flex-1 !border-0 !bg-transparent !outline-none !shadow-none text-xs text-[var(--inno-text)] placeholder:text-[var(--inno-text-subtle)] focus:border-0 focus:outline-none focus:ring-0 focus:shadow-none focus-visible:!border-0 focus-visible:!outline-none focus-visible:!shadow-none focus-visible:ring-0"
					/>
					{query ? (
						<button
							type="button"
			...
			<div className="max-h-[50vh] overflow-y-auto rounded-md">
				{isLoading ? (
					<div className="flex items-center justify-center gap-2 py-6 text-xs text-[var(--inno-text-muted)]"><Spinner size={14} />{t("common.loading")}</div>
				) : totalMatched === 0 ? (
