...
describe("ChatStore stream ownership", () => {
	...
	it("clears retry state when leaving the current conversation", () => {
		const store = new ChatStoreImpl();
		store.lastUserPrompt = "old prompt";
		store.lastImages = [{ data: "image-data", mimeType: "image/png" }];

		store.clear();

		expect(store.lastUserPrompt).toBeNull();
		expect(store.lastImages).toBeUndefined();
	});
